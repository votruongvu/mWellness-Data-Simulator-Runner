...
order: 3
title: "Add mobile safety guardrails and human gates"
depends_on: ["MWR-FW-002"]
status: done
---

# MWR-FW-003 — Add Mobile Safety Guardrails And Human Gates

## Story File

`STORY-add-mobile-safety-guardrails-and-human-gates.md`
...
Add mandatory mobile-native safety guardrails, approval gates, and risk docs for health data writes and backend authority.
...
- Add no accidental health data write guardrail.
- Add no real write without dry-run, capability check, permission check, and explicit confirmation.
- Add no fake native write success guardrail.
- Add no silent HealthKit / Health Connect permission prompt guardrail.
- Add no unsupported metric silently ignored guardrail.
- Add no raw token/log leakage guardrail.
- Add no backend authority bypass guardrail.
- Add no mock test cases/scenarios marked as complete product behavior guardrail.
- Add no Google Fit and no direct vendor SDK unless human-approved.
- Update `known-risks.md`, `human-approval-gates.md`, `source-of-truth.md`, and relevant checklists.
...
- P0 safety guardrails are documented in framework source-of-truth or CLAUDE.md.
- Human approval gates cover native writes, permission model, safety bypass, token storage, backend contract gaps, new platform/vendor, and production readiness claims.
- Known risks include accidental writes, fake success, permission flow, token/log leakage, backend bypass, and metric support mismatch.
- Safety rules are visible to future story generation.
- No native write implementation is added.
...
`MR-FRAMEWORK-00 STORY-add-mobile-safety-guardrails-and-human-gates`
...
- **Evidence:** CLAUDE.md 'Mandatory safety guardrails'; adapter/human-approval-gates.md (10 hard gates); adapter/known-risks.md (R-MWR-001..016; P0 accidental-write/fake-success/token-leak); adapter/project-source-of-truth.md safety invariants; framework/rules/gates.md; crown-jewel checklists.
- **Deliverables provenance:** framework artifacts were produced in the MR-FRAMEWORK-00 bootstrap commits `cbe7e22` / `8ef3a72` (now on `main`, ancestor of this branch) and re-verified live in this loop; no new files required (guardrails + gates already documented).
